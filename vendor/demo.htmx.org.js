(function () {
  ...
  function interpolate(str, params) {
    try {
      ...
      return eval(`env => { with (env) { return \`${escapedCode}\` } }`)(
        params,
      );
    } catch (e) {
      ...
      document.querySelectorAll("template").forEach(function (elt) {
        if (elt.getAttribute("url")) {
          ...
          MockRequests.setDynamicMockUrlResponse(elt.getAttribute("url"), {
            dynamicResponseModFn: function (request, response, parameters) {
              log(
                "demo:request",
                "A mock request was made: ",
                request,
                response,
                parameters,
              );
              return interpolate(elt.innerHTML, {
                ...parameters,
                ...Object.fromEntries(new URLSearchParams(request)),
              });
            },
            delay: delay,
